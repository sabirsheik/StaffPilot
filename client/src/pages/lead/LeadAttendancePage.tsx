...
export const LeadAttendancePage = () => {
  ...
  const interns = (dashboardStats?.teamMembers || []).map((m) => ({
    value: m._id || m.id,
    label: m.fullName,
  }));
  ...
  const historyParams = useMemo(
    ...
    [page, pageSize, q, userId, sortBy, sortDir]
  );
  ...
  const tabs = [
    { key: 'today', label: "Today's Team", icon: CalendarDays },
        ...
        <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
          ...
          <div className="stat-card">
            <div className="flex items-start justify-between">
              ...
              <div className="h-11 w-11 rounded-xl border border-amber-200 bg-amber-50 flex items-center justify-center text-amber-600 shadow-sm">
          ...
          <div className="stat-card">
            <div className="flex items-start justify-between">
              <div>
                ...
                <p className="mt-2 text-3xl font-bold text-slate-900 tracking-tight">
                  {monthly?.totalWorkedHours || 0}
                </p>
                <p className="mt-1 text-xs text-slate-500">
                  {monthly?.totalExpectedHours || 0}h expected
                </p>
              </div>
              <div className="h-11 w-11 rounded-xl border border-brand-200 bg-brand-50 flex items-center justify-center text-brand-600 shadow-sm">
        ...
        {monthly?.daily?.length ? (
          <div className="card p-5 sm:p-6">
            <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-3 mb-5">
              <div>
                <p className="text-sm font-semibold text-slate-900">Team Attendance Trend</p>
                    ...
                    >
                      ...
                      >
                        <div className="bg-slate-900 text-white text-[11px] font-medium rounded-lg px-2.5 py-1.5 shadow-lg whitespace-nowrap -translate-y-full">
                          <div className="tabular-nums">{d.present} checked-in</div>
    ...
    <div className="p-6 lg:p-8 max-w-[1600px] mx-auto space-y-6">
      <div className="flex flex-col sm:flex-row sm:items-end sm:justify-between gap-4">
        <div>
          ...
          <p className="mt-1 text-sm text-slate-500">
            Track your own attendance and review your team's check-ins.
          </p>
        </div>
      ...
      {tab === 'today' ? (
        <div className="grid grid-cols-1 xl:grid-cols-2 gap-6">
          <div className="card overflow-hidden">
            <div className="border-b border-slate-200 px-6 py-4">
              ...
              <p className="text-xs text-slate-500 mt-0.5">
                Team members who have checked in
              </p>
            </div>
            {overviewLoading ? (
              ...
              <div className="divide-y divide-slate-200">
                {overview.records.map((r) => {
                  const u = r.user || {};
                  const Icon = u.role === ROLES.TEAM_LEAD ? UserCog : UserIcon;
                  return (
                    <div key={r._id || r.id} className="flex items-center justify-between px-6 py-3 gap-3">
                      <div className="flex items-center gap-3 min-w-0">
                        <div className="h-9 w-9 rounded-full bg-emerald-50 text-emerald-700 border border-emerald-200 flex items-center justify-center text-sm font-medium shrink-0">
                          {u.fullName?.charAt(0)?.toUpperCase() || 'U'}
                        </div>
                        <div className="min-w-0">
                          <p className="text-sm font-medium text-slate-900 truncate">{u.fullName}</p>
                          <p className="text-xs text-slate-400 truncate">
                            <span className={`badge-role ${ROLE_BADGE_CLASS[u.role]} !text-[10px] mr-1`}>
          ...
          <div className="card overflow-hidden">
            <div className="border-b border-slate-200 px-6 py-4">
              <h3 className="text-base font-semibold text-slate-900">
                Pending Check-in{overview?.totalAbsent ? ` · ${overview.totalAbsent}` : ''}
              </h3>
              <p className="text-xs text-slate-500 mt-0.5">
                Awaiting check-in
              </p>
            ...
            {overviewLoading ? (
              ...
              <div className="text-center py-12">
                ...
                <p className="text-sm font-medium text-emerald-700">All checked in</p>
                <p className="text-xs text-slate-400 mt-1">
                  Everyone on the team has marked their attendance.
                </p>
              </div>
            ) : (
              <div className="divide-y divide-slate-200 max-h-[480px] overflow-y-auto">
                {overview.absentUsers.map((u) => (
                  <div key={u._id || u.id} className="flex items-center justify-between px-6 py-3 gap-3">
                    <div className="flex items-center gap-3 min-w-0">
                      <div className="h-9 w-9 rounded-full bg-red-50 text-red-700 border border-red-200 flex items-center justify-center text-sm font-medium shrink-0">
                        {u.fullName?.charAt(0)?.toUpperCase() || 'U'}
                      </div>
                      <div className="min-w-0">
                        <p className="text-sm font-medium text-slate-900 truncate">{u.fullName}</p>
                    ...
                    <span className="inline-flex items-center gap-1.5 px-2.5 py-0.5 rounded-full text-xs font-medium border text-amber-700 bg-amber-50 border-amber-200 shrink-0">
        ...
        <DataTable
          ...
          subtitle={tab === 'history' ? 'Attendance records for you and your team' : `Attendance summary for ${monthly?.monthLabel || ''}`}
          ...
          emptyState={
            <div className="text-center py-14">
              ...
              <p className="text-xs text-slate-400 mt-1 max-w-sm mx-auto">
                Try a different filter or team member.
              </p>
            </div>
          }
        />
...
export default LeadAttendancePage;
